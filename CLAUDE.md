...
- `game/`
  ...
  - `Packs.kt` — сборки из GitHub-репозиториев (`Packs.official`: id, repo, а также `tagline`/`description`/`highlights` для каталога). `sync` перед запуском ставит последний релиз: файлы репозитория копируются (правки игрока сохраняются, пока файл не изменился в сборке), внешние файлы из `mint-pack.json` качаются по url+sha1. Папка с `.git` — рабочая копия разработчика, не синхронизируется. `writeManifest` ищет моды/шейдеры/ресурспаки на Modrinth по sha1. `PackSide { BOTH, CLIENT, SERVER }` в манифесте делит моды между клиентом и сервером; выставленная вручную сторона переживает пересборку манифеста.
...
- `ui/`
  ...
  - `HomeTab.kt` — hero с кнопками «Играть» и «Сервер» и прогрессом (консоли игры пока нет — лог в `logs/game-latest.log`), карточки «Мои сборки» (иконки, быстрый выбор, «все» → вкладка «Сборки»), «Что нового», «Ресурсы» (память, Java, GPU, CPU, ОС). Фон hero — превью сборки (затемнение снизу + светлые цвета `OnArt*`), без превью — мятный градиент.
  - `InstancesTab.kt` — вкладка «Сборки» (кубик): прокручиваемые карточки с превью, иконкой, плашками версий, описанием и кнопкой «Выбрать» (делает сборку активной и возвращает на главную, игру не запускает).
  - `PackArt.kt` — картинки сборок с кэшем и `PackIcon` (без картинки — буква на мятной плитке). Официальные вшиты в лаунчер: `resources/packs/<id>/icon.png` (квадрат 256) и `banner.jpg` (~1600 px); у локальных — `icon.png`/`banner.png` в папке сборки.
...
- Тестовая сборка — **TestMint** (`run/instances/testmint`, репозиторий `GrayRK/TestMint`, NeoForge 1.21.1): полигон для модов и функций лаунчера, журнал там же в `MODPACK.md`. Новые моды и эксперименты — сначала сюда.
- Иконки, превью и описания сборок живут в лаунчере, а не в репозиториях сборок: каталог показывает их до скачивания.
...
- Боковая панель видна всегда. Вкладка «Моды» (пазл) пока ничего не делает — запланировано управление сборкой (моды, ресурспаки).
